...
function _track(category, action, opt_label, opt_value) {
...
function send_local(data) {
    ...
    var settings = {
        ...
        complete: function (res) {
            res_header = kv_to_array(res.getAllResponseHeaders());
            res_cookie = '';
...
function response_view() {
    ...
    _track('getman', send_type, method + ' ' + url + ' ' + res_status);
...
$("#btn_send").click(function () {
    ...
    if (url && url.length > 0) {
        ...
        $("#btn_response_body").attr('preview','false');
...
function iframe_load() {
    var iframe_height = document.getElementById("iframe_body").contentWindow.document.documentElement.scrollHeight;
    $("#iframe_body").attr('height', iframe_height);
}
...
$("#btn_response_body").click(function () {
    ...
    if ($(this).hasClass("active")) {
        if ($(this).attr('preview') == "true") {
            ...
            $("#txt_response_body").html('<iframe id="iframe_body" scrolling="no" frameborder="0" width="100%" onload="iframe_load(this)" sandbox="allow-forms allow-scripts allow-same-origin allow-popups"></iframe>');
            var base_url = url.substring(0, url.lastIndexOf("\/"));
            var iframe_body = res_body;
            iframe_body = iframe_body.replace(/href="(?=(?!http|\/\/))/,'href="' + base_url + '/');
            iframe_body = iframe_body.replace(/src="(?=(?!http|\/\/))/,'src="' + base_url + '/');
            $("#iframe_body").attr('srcdoc', iframe_body);
        }
    }
